...
import { Users, Building, Music, User, BarChart3, TrendingUp, Calendar } from 'lucide-react';
...
export default function DashboardAdministrador() {
  ...
  const processMonthlyData = (casas, artistas, clientes) => {
    const allUsersData = [
      ...casas.map(u => ({ 
        ...u, 
        tipo: 'CASASHOW', 
        data: u.data || u.createdAt || u.created_at || new Date().toISOString()
      })),
      ...artistas.map(u => ({ 
        ...u, 
        tipo: 'ARTISTA', 
  ...
  const fetchAllStats = async () => {
    setLoading(true);
    try {
      const [casas, artistas, clientes] = await Promise.all([
      ...
      setStats({
        totalUsuarios: casasCount + artistasCount + clientesCount,
        casasDeShow: casasCount,
        artistas: artistasCount,
        clientes: clientesCount,
      });
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {monthlyData.map((month, index) => {
                ...
                if (prevMonth) {
                  if (prevMonth.total === 0 && month.total > 0) {
                    showGrowth = true;
                    growth = 100;
                  } else if (prevMonth.total > 0) {
                    showGrowth = true;
                    growth = ((month.total - prevMonth.total) / prevMonth.total) * 100;
                  }
                }
