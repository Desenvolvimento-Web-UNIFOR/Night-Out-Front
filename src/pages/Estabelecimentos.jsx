    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {items.map((establishment, index) => (
          <motion.div key={establishment.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.05 }} whileHover={{ scale: 1.01, y: -2 }} className="glass p-6 hover:glow-primary transition-all duration-300">
            <div className="flex flex-col lg:flex-row gap-6">
              <div className="relative lg:w-80 h-48 lg:h-64">
                <img src={establishment.image} alt={establishment.name} className="w-full h-full object-cover rounded-xl" />
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-text mb-3">{establishment.name}</h3>
                  <p className="text-muted text-lg mb-4 leading-relaxed">{establishment.description}</p>
                ...
                <div className="flex justify-end">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center space-x-2 btn-primary px-8 py-3 glow-primary">
