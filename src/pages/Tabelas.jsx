...
export default function Tabelas() {
  ...
  const art = usePaginatedData({ mode: "local", localData: artistsData, pageSize: 5 });
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="glass">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar funcionários..." className="pl-10 pr-4 py-2 bg-panel border border-border rounded-lg text-text placeholder-muted focus-ring w-64" />
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left py-4 px-6 text-muted font-medium">Admissão</th>
                <th className="text-left py-4 px-6 text-muted font-medium">Ações</th>
              </tr>
            </thead>
            <tbody>
              {emp.items.map((employee, index) => (
                <motion.tr key={employee.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.05 }} className="border-b border-border hover:bg-panel/30 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <span className={employee.status === 'Online' ? 'chip-success' : 'chip-offline'}>{employee.status}</span>
                  </td>
                  <td className="py-4 px-6 text-muted">{employee.joinDate}</td>
                  <td className="py-4 px-6">
                    <div className="flex items-center space-x-2">
                      <button className="p-2 hover:bg-panel rounded-lg transition-colors focus-ring"><Edit size={16} className="text-muted hover:text-text" /></button>
                      <button className="p-2 hover:bg-panel rounded-lg transition-colors focus-ring"><MoreVertical size={16} className="text-muted hover:text-text" /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
        ...
        <div className="p-4">
          <Pagination page={emp.page} pageSize={emp.pageSize} total={emp.total} onPageChange={emp.setPage} />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="glass">
        <div className="p-6 border-b border-border">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <Search size={18} className="absolute left-3 top-3 text-muted" />
                <input type="text" placeholder="Buscar artistas..." className="pl-10 pr-4 py-2 bg-panel border border-border rounded-lg text-text placeholder-muted focus-ring w-64" />
              </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {art.items.map((artist, index) => (
                <motion.tr key={artist.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.05 }} className="border-b border-border hover:bg-panel/30 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center space-x-3">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center space-x-3">
                      <div className="w-24 bg-panel rounded-full h-2">
                        <div className="h-2 bg-primary rounded-full transition-all duration-500" style={{ width: `${artist.progress}%` }} />
                      </div>
        ...
        <div className="p-4">
          <Pagination page={art.page} pageSize={art.pageSize} total={art.total} onPageChange={art.setPage} />
        </div>
      </motion.div>
    </div>
  );
